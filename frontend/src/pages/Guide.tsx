...
export function Guide() {
  ...
  return (
    <div className="px-9 pt-7 pb-15">
      ...
      <div className="sticky top-0 z-10 -mx-9 px-9 bg-bg pb-3.5 mb-4 flex flex-wrap gap-1.5 border-b border-border">
        {SECTIONS.map((s) => {
          const Icon = s.icon
          return (
            <button
              ...
              className={`flex items-center gap-1.5 text-left text-[13px] px-3 py-1.5 rounded-full border-none cursor-pointer transition-colors ${
                activeId === s.id ? 'bg-accent/15 text-text font-semibold' : 'bg-input text-muted hover:text-text'
              }`}
            >
