...
function TxCard({ tx }: { tx: Transaction }) {
  return (
    <div className="border border-border rounded-2lg p-3.5">
      <div className="text-md font-semibold">{tx.raw_description}</div>
      <div className="text-xs text-muted my-1">
        {fmtDate(tx.transaction_date)} · {tx.category} · {tx.bank_name} {tx.account_number_masked}
      </div>
      <div className={`font-mono text-title-sm ${tx.amount > 0 ? 'text-success' : 'text-text'}`}>
        {fmtSigned(tx.amount)}
      </div>
    </div>
  )
    ...
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-[380px] bg-card shadow-2xl z-50 p-6 overflow-y-auto border-l border-border">
        <div className="flex justify-between items-center mb-4.5">
          <div className="text-title-sm font-bold">Refund Pairing</div>
